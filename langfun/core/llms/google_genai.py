...
class GenAI(lf.LanguageModel):
    ...
    model: Annotated[
        Literal["gemini-1.5-pro-002", "gemini-1.5-flash-002"],
        "Model name.",
    ]
    ...
    supported_modalities: Annotated[
        list[str], "A list of MIME types for supported modalities"
    ] = []
    ...
    def _content_from_message(self, prompt: lf.Message) -> list[str | BlobDict]:
                ...
                try:
                    ...
                    for modality in modalities:
                        if modality.is_text:
                            ...
                            chunk = BlobDict(
                                data=modality.to_bytes(), mime_type=modality.mime_type
                            )
                        chunks.append(chunk)
...
class _LegacyGenerativeModel(pg.Object):
    ...
    @abc.abstractmethod
    def generate(self, prompt: str, generation_config: GenerationConfig) -> pg.Dict:
...
class _ModelHub:
    ...
    def get(self, model_name: str) -> GenerativeModel | _LegacyGenerativeModel:
